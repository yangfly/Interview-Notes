...
```
因此我们需要三个依赖变量 (buy[i-1], sell[i-2], sell[i-1]) 和一个额外转移变量 buy[i]
...
int maxProfit(vector<int>& prices) {
    if (prices.size() < 2) return 0;
    int dp[prices.size() + 1]; // add 1 more to guard
    dp[0] = 0;
    buy = -prices[0];
    for (int i = 2; i < prices.size(); i++) {
        buy = max(buy, dp[i-1] - prices[i]);
        dp[i+1] = max(dp[i], buy + prices[i]);
    }
